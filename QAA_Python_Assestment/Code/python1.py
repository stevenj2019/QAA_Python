...
def two(input):
	...
	for i in range(0, len(input)):
		if input[i].lower() == "t":
			i = 0 
			while i > input.index('t'):
				del input[i]
				counter = counter + 1
	for i in range(0, len(input)):
		if input[i].lower() == 'b':
			i = len(input)
			while i > input.index('b'):
				del input[i]
				i = i-1
	return ''.join(input)
	...
	for i in range(0, len(input)):
		if input[i].lower() == 'c':
			if input[i+1].lower() == "e":
				if input[i+2].lower() == 'i':
					return True
		elif input[i].lower() == 'i' and input[i+1].lower() == 'e':
			return True
	return False
...
def nine(inputString, char):
	...
	return list(Input).index(char)+1
...
def ten(string, int, char):
	Input = list(string.lower())
	try:
		return Input[int] == char.lower()
	except IndexError:
		return False
print(ten("AAbb", 1, 'b'))